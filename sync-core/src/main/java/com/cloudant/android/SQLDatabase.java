...
/**
 * Abstraction of the underneath SQLite database that DB is built upon. Most of the methods are
 * copied from <code>android.database.sqlite.SQLiteDatabase</code>.
 *
 * The implementation should be thread safe. If a connection pool is used, the connection pool is managed underneath
 * and be transparent to client.
 *
 * The client should create only on instance, which means the underneath SQLite database is only opened once. The
 * behavior of multiple SQLDatabase instances to the same database file is undefined and not recommended.
 *
 * When SQLDatabase is shared, all transaction are serialized, so keep your transaction as small as possible.
 *
 * @see com.cloudant.sync.sqlite.android.AndroidSQLite
...
public interface SQLDatabase {
     ...
     */
    public int update(String table, com.cloudant.sync.sqlite.ContentValues values,
     ...
     */
    public long insert(String table, com.cloudant.sync.sqlite.ContentValues values);
