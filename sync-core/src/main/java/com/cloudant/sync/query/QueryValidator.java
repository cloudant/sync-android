...
class QueryValidator {
    ...
    public static Map<String, Object> normaliseAndValidateQuery(Map<String, Object> query) {
        ...
        // and make
        //     [ {"field1": { "$eq": "mike"} }, ... } ]
        String compoundOperator = query.keySet().iterator().next();
        ArrayList<Object> predicates = new ArrayList<Object>();
        if (query.get(compoundOperator) instanceof List) {
            predicates = addImplicitEq((ArrayList<Object>) query.get(compoundOperator));
        }
    ...
    @SuppressWarnings("unchecked")
    private static ArrayList<Object> addImplicitEq(ArrayList<Object> clause) {
        ...
        for (Object fieldClause: clause) {
            ...
            if (fieldClause instanceof Map && !((Map) fieldClause).isEmpty()) {
                ...
                fieldName = fieldClauseMap.keySet().iterator().next();
            ...
            if (!fieldName.startsWith("$")) {
                ...
                predicate = addImplicitEq((ArrayList<Object>) predicate);
            }
    ...
    private static boolean validateSelector(Map<String, Object> selector) {
        String topLevelOp = selector.keySet().iterator().next();
        ...
        if (topLevelOp.equals(AND) || topLevelOp.equals(OR)) {
            ...
            if (topLevelArg instanceof List) {
                ...
                return validateCompoundOperatorClauses((ArrayList<Object>) topLevelArg);
            }
        }
