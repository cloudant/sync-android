...
public class DatastoreSchemaTests {
     ...
     * with two attachments.
     */
    public void migrationToAtLeast100OnDatastoreOpen() throws DocumentStoreException,
            ExecutionException,
    ...
    // check we can open a 1.x index database (generated by canQueryUsingPorterTokenizerStemmer)
    @Test
    public void openLegacyIndexDatabase() throws DocumentStoreException {
        File temp_folder = new File(TestUtils.createTempTestingDir(this.getClass().getName()));
        File legacyDbZip = TestUtils.loadFixture("fixture/legacy_database_with_indexes.zip");
        Assert.assertTrue(unzipToDirectory(legacyDbZip, temp_folder));
        final File dbPath = new File(temp_folder.getAbsolutePath(), "AbstractQueryTestBase");
        DocumentStore ds = DocumentStore.getInstance(dbPath);
        Assert.assertThat(ds.query().listIndexes(), hasSize(1));
        Index i = ds.query().listIndexes().get(0);
        Assert.assertThat(i.indexName, is("basic_text"));
        // fields are "comment", "_id", "_rev"
        Assert.assertThat(i.fieldNames, hasSize(3));
        Assert.assertThat(i.fieldNames, hasItem(new FieldSort("comment", FieldSort.Direction
                .ASCENDING)));
        Assert.assertThat(i.tokenizer, is(new Tokenizer("porter")));
    }
