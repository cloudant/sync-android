...
public class PickWinningRevisionCallable implements SQLCallable<Void> {
    ...
    public Void call(SQLDatabase db) throws DocumentStoreException {
        ...
        try {
            ...
            throw new DocumentStoreException("Exception thrown whilst trying to fetch non-deleted " +
                    "leaf nodes.", sqe);
        } finally {
        ...
        if (leafs.size() == 0) {
            try {
                ...
                throw new DocumentStoreException("Exception thrown whilst trying to fetch all leaf " +
                        "nodes.", sqe);
            } finally {
                DatabaseUtils.closeCursorQuietly(cursor);
            }
