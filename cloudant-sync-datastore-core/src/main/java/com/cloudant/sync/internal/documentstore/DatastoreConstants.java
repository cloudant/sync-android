...
class DatastoreConstants {
    ...
    public static String[] getSchemaVersion3() {
        return new String[] {
            "    CREATE TABLE docs ( " +
            "        doc_id INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY, " +
            "        docid LONGVARCHAR UNIQUE NOT NULL); ",
            "    CREATE INDEX docs_docid ON docs(docid); ",
            "    CREATE TABLE revs ( " +
            "        sequence INTEGER GENERATED BY DEFAULT AS IDENTITY (START WITH 1) PRIMARY KEY, " +
            "        doc_id INTEGER NOT NULL REFERENCES docs(doc_id) ON DELETE CASCADE, " +
            "        parent INTEGER , " +
            "        current BOOLEAN, " +
            "        deleted BOOLEAN DEFAULT 0, " +
            "        available BOOLEAN DEFAULT 1, " +
            "        revid LONGVARCHAR NOT NULL, " +
            "        json BLOB); ",
            ...
            "        remote LONGVARCHAR NOT NULL, " +
            "        startPush BOOLEAN, " +
            "        last_sequence LONGVARCHAR, " +
    ...
    public static String[] getSchemaVersion200() {
        return new String[]{
                ...
                "        json BLOB, " +
                "        UNIQUE (doc_id, revid)); ",
                "    INSERT INTO new_revs ( doc_id, parent, current, deleted, available, revid, json) SELECT DISTINCT revs.doc_id, revs.parent, revs.current, revs.deleted, revs.available, revs.revid, revs.json FROM revs; ",
                "    DROP TABLE revs CASCADE; ",
                ...
                "        UNIQUE (sequence, filename)); ",
                "    INSERT INTO new_attachments (sequence, filename, key, type, length, revpos, encoding, encoded_length) SELECT DISTINCT attachments.sequence, attachments.filename, attachments.key, attachments.type, attachments.length, attachments.revpos, attachments.encoding, attachments.encoded_length FROM attachments; ",
